...
interface ProcessStep { n: string; icon: string; title: string; time: string; desc: string }
interface Testimonial { name: string; company: string; text: string; result: string; initials: string }
...
function getQRCodeUrl(bankIban: string, amount: number, vs: string): string {
  const paymentString = `SPD*1.0*ACC:${bankIban}*AM:${amount}.00*CC:CZK*MSG:Web webdo24.cz ${vs}*X-VS:${vs}`;
...
export default function HomeClient({
  ...
  useEffect(() => {
    ...
    const expiry = stored
      ? parseInt(stored, 10)
      : (() => {
          const t = Date.now() + timerSeconds * 1000;
  ...
  const currentPrice = expired ? regularPrice : salePrice;
            ...
            {paymentMethod === 'qr' && (
              <div className="text-center">
                ...
                <div className="flex justify-center mb-4">
                  <img
                    src={getQRCodeUrl(bankIban, orderData.amount, orderData.vs)}
                ...
                <div className="bg-zinc-800 rounded-xl p-4 space-y-2 text-sm text-left">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-mono">{bankAccountDisplay}</span>
                  </div>
                  <div className="flex justify-between">
            ...
            {paymentMethod === 'bank' && (
              <div>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center py-2 border-b border-zinc-700">
                    <span className="text-zinc-400">Číslo účtu:</span>
                    <span className="text-white font-mono font-bold">{bankAccountDisplay}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-zinc-700">
                    <span className="text-zinc-400">IBAN:</span>
                    <span className="text-white font-mono text-xs">{bankIban}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-zinc-700">
                    <span className="text-zinc-400">Variabilní symbol:</span>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      >
        {expired ? (
          <span className="text-zinc-400 text-sm font-semibold">
            Speciální nabídka vypršela · Cena: {fmtPrice(regularPrice)}
          </span>
        ) : timeLeft === null ? (
          <span className="text-yellow-400 font-bold text-base">
            ⏱&nbsp;Speciální cena {fmtPrice(salePrice)} — načítám…
          </span>
        ) : (
          <span className={`font-mono font-bold text-base md:text-lg ${urgent ? 'text-red-300' : 'text-yellow-400'}`}>
            ⏱&nbsp;Speciální cena {fmtPrice(salePrice)} vyprší za&nbsp;
      ...
      <section className="pt-24 pb-16 px-4 text-center max-w-3xl mx-auto">
        ...
        <h1 className="text-4xl md:text-6xl font-black leading-[1.0] tracking-tight mb-6 whitespace-pre-line">
          {heroHeading}
        </h1>
        <p className="text-xl md:text-2xl text-zinc-300 mb-4 whitespace-pre-line">
          {heroSubheading}
        </p>
        ...
        {!expired && timeLeft !== null && (
          <p className="mt-5 text-zinc-400 text-sm">
            Speciální cena{' '}
            <strong className="text-yellow-400">{fmtPrice(salePrice)}</strong>{' '}
            platí jen{' '}
            <span className={`font-mono font-bold ${urgent ? 'text-red-400' : 'text-white'}`}>
              {fmt(timeLeft)}
            </span>
            {'. '}Pak se vrátí na {fmtPrice(regularPrice)}.
          </p>
        )}
      </section>
        ...
        >
          {!expired ? (
            <div className="flex items-baseline justify-center gap-4">
              <span className="text-zinc-500 line-through text-2xl">{fmtPrice(regularPrice)}</span>
              <span className="text-yellow-400 font-black text-6xl">{fmtPrice(salePrice)}</span>
            </div>
          ) : (
            <span className="text-white font-black text-6xl">{fmtPrice(regularPrice)}</span>
          )}
          {!expired && (
            <p className="text-zinc-500 text-sm mt-2">
              Ušetříš {fmtPrice(regularPrice - salePrice)} oproti běžné ceně.
            </p>
          )}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 p-4 bg-zinc-950/95 backdrop-blur-sm border-t border-zinc-800">
        ...
        >
          {expired ? `Profesionální web — ${fmtPrice(regularPrice)}` : `Speciální cena ${fmtPrice(salePrice)} — objednat →`}
        </a>
      </div>
    </div>
  );
